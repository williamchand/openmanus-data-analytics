...
def test_ask_ai_carrier_query(client):
    res = client.post("/api/ask-ai", json={"question": "Which carrier has the highest delay rate?"})
    assert res.status_code == 200
    data = res.json()
...
def test_ask_ai_forecasting_query(client):
    res = client.post("/api/ask-ai", json={"question": "Predict demand for SKU PAPER-0197 for the next 4 months"})
    assert res.status_code == 200
    data = res.json()
